...
contract DSCEngine is ReentrancyGuard {
    ...
    mapping(address user => uint256 mintedDsc) private s_mintedDsc;
    ...
    event CollateralRedeemed(
        address indexed user, address indexed redeemedFrom, address indexed redeemedTo, uint256 amount
    );
        ...
        nonReentrant
    {
        ...
        bool succcess = IERC20(tokenCollateralAddress).transferFrom(msg.sender, address(this), amountCollateral);
        if (!succcess) {
        ...
        moreThanZero(amountCollateral)
    {
        ...
        // health factor already checked in redeemCollateral
    }
    ...
    function getTokenAmountFromUsd(address token, uint256 amountUsd) public view returns (uint256) {
        ...
        return (amountUsd * PRECISION) / (uint256(price) * ADDITIONAL_FEED_PRECISION);
    }
    ...
    function getAccountCollateralValue(address user) public view returns (uint256 totalCollateral) {
        for (uint256 i = 0; i < s_collateralTokens.length; i++) {
            address token = s_collateralTokens[i];
            uint256 amount = s_collateralBalances[user][token];
            totalCollateral += getUSDValue(token, amount);
        }
